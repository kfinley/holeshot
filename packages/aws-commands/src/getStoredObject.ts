...
export class GetStoredObjectCommand implements Command<GetStoredObjectRequest, GetStoredObjectResponse> {
  ...
  async runAsync(params: GetStoredObjectRequest): Promise<GetStoredObjectResponse> {
    ...
    try {
      ...
      const endpoint = await getEndpointFromInstructions(command.input as unknown as Record<string, unknown>, GetObjectCommand as EndpointParameterInstructionsSupplier, this.s3Client.config);
